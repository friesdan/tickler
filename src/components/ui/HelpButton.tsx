...
export function HelpButton({ onStartTour }: HelpButtonProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-11 h-11 rounded-full glass flex items-center justify-center text-white/40 hover:text-white/70 cursor-pointer transition-colors text-sm font-bold"
      ...
      {open && (
        <div id="help-menu" role="menu" className="absolute bottom-full right-0 mb-2 w-56 glass px-4 py-3 text-xs animate-fadeInUp">
          ...
          <div className="border-t border-white/10 mt-2 pt-2">
            <div className="text-white/40 font-bold uppercase tracking-wider mb-1.5 text-[10px]">
              Keyboard Shortcuts
            </div>
            {KEYBOARD_SHORTCUTS.map((s) => (
              <div key={s.key} className="flex justify-between py-0.5">
                <kbd className="text-white/50 bg-white/[0.06] px-1.5 py-0.5 rounded text-[10px]">{s.key}</kbd>
